...
export default function AdminPanel({ race, entriesCount, onStartRace, onNewRace, isRacing }) {
  ...
  const handleNewRace = () => {
    ...
    if (isMass) {
      ...
      onNewRace(buyInNum, spotsNum, duration, { isMassRace: true, auto_start: autoStart });
    } else {
      const lanesNum = Math.min(20, Math.max(2, Number(lanes) || 2));
      const dplNum = Math.max(1, Number(ducksPerLane) || 1);
      onNewRace(buyInNum, lanesNum, duration, { ducks_per_lane: dplNum, auto_start: autoStart });
    }
  };
